 ...
 * streams match the recorded ones is fed the recorded buffers, results and
 * capture events at the intervals they arrived at: the pre-roll on a loop
 * while the repeating request runs, and the burst once for each still capture.
...
struct atl_camera {
	...
	bool running;
	int repeating_id;
	GQueue *oneshots; /* pending one-shot request ids */
	int64_t frame_number;
	int64_t clock; /* the sensor timestamp the next segment starts at */
	/* Camera1 */
...
static bool load_file(const char *path)
{
	...
	while (atl_rec_read_chunk(file, &type, &data, &len)) {
		...
		case ATL_REC_REQUEST:
			break; /* kept in the file for the eye, not replayed */
	...
	if (!rep.n_cameras || !rep.n_streams || !frames) {
 ...
 * frame comes round again on the next loop.
 */
...
/* whether a still capture is waiting behind the pre-roll */
static bool oneshot_waiting(struct atl_camera *camera)
{
	bool waiting;

	g_mutex_lock(&camera->lock);
	waiting = !g_queue_is_empty(camera->oneshots);
	g_mutex_unlock(&camera->lock);
	return waiting;
}

/*
 * One pass over [from, to): the recorded intervals, at the recorded rate. The
 * pre-roll gives way at a frame boundary when a still capture is waiting -
 * a shutter that took a whole loop of the pre-roll to answer would be the one
 * thing about the replay that felt nothing like a camera.
 */
static void play_segment(struct atl_camera *camera, int from, int to, int request_id,
                         bool interruptible)
{
	int64_t first = 0, shift = 0, started_at = g_get_monotonic_time();
	...
	for (int i = from; i < to; i++) {
		const struct replay_event *event = &rep.events[i];
		int64_t timestamp;
		int app_stream;
		...
		if (!have_first && (event->type == ATL_REC_STARTED || event->type == ATL_REC_BUFFER)) {
			first = event->timestamp;
			shift = camera->clock - first;
			have_first = true;
		}
		...
		switch (event->type) {
		case ATL_REC_STARTED: {
			int64_t due = (int64_t)((event->timestamp - first) / 1000 / rep.speed);

			if (interruptible && oneshot_waiting(camera))
				return;

			/* the recorded interval, off the wall clock this pass started on */
			while (g_get_monotonic_time() - started_at < due) {
				g_usleep(1000);
				g_mutex_lock(&camera->lock);
				if (!camera->running) {
					g_mutex_unlock(&camera->lock);
		...
		case ATL_REC_BUFFER:
			app_stream = app_stream_of(camera, event->stream);
		...
		case ATL_REC_RESULT:
			if (session)
				camera->cbs.result(camera->user, request_id, camera->frame_number,
				                   result_at(event, timestamp));
			break;
		case ATL_REC_FAILED:
			if (session)
				camera->cbs.failed(camera->user, request_id, camera->frame_number);
			break;
		case ATL_REC_LOST:
			app_stream = app_stream_of(camera, event->stream);
			if (session && app_stream >= 0)
				camera->cbs.buffer_lost(camera->user, request_id, camera->frame_number,
				                        app_stream);
			break;
		}
		if (event->type == ATL_REC_STARTED || event->type == ATL_REC_BUFFER)
			camera->clock = timestamp + 1;
	}
}
...
static gpointer play_thread(gpointer data)
{
	...
	int pre_to = rep.burst_at >= 0 ? rep.burst_at : rep.n_events;
	...
	for (;;) {
		...
		if (!g_queue_is_empty(camera->oneshots))
			oneshot = GPOINTER_TO_INT(g_queue_pop_head(camera->oneshots)) - 1;
		repeating = camera->repeating_id;
		g_mutex_unlock(&camera->lock);

		if (oneshot >= 0 && rep.burst_at >= 0)
			play_segment(camera, rep.burst_at, rep.n_events, oneshot, false);
		else if (oneshot >= 0)
			play_segment(camera, 0, pre_to, oneshot, false);
		else if (repeating >= 0)
			play_segment(camera, 0, pre_to, repeating, true);
	}
}
                                  ...
                                  bool repeating)
{
	 ...
	 * for. The requests themselves are in the recording, to be read */
	(void)settings;
	(void)streams;
	g_mutex_lock(&camera->lock);
	if (repeating) {
		...
		/* the queue holds ids, and 0 is one: everything goes in shifted */
		g_queue_push_tail(camera->oneshots, GINT_TO_POINTER(request_id + 1));
	}
	g_mutex_unlock(&camera->lock);
	play_start(camera);
	return true;

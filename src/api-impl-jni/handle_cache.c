...
/*
 * A class that is not there is fatal here: every entry below it would be looked
 * up on a NULL jclass, which crashes the VM rather than throwing.
 *
 * A member that is not there only leaves a NoSuchMethodError/NoSuchFieldError
 * pending, and it is left pending on purpose -- GetMethodID returns NULL without
 * running once an exception is pending, so the first miss is the one the caller
 * sees when it checks after this returns.
 */
void set_up_handle_cache(JNIEnv *env)
{
	handle_cache.activity.class = _REF(atl_find_class_or_exit(env, "android/app/Activity"));
	handle_cache.activity.onCreate = _METHOD(handle_cache.activity.class, "performCreate", "(Landroid/os/Bundle;)V");
	handle_cache.activity.onPostCreate = _METHOD(handle_cache.activity.class, "onPostCreate", "(Landroid/os/Bundle;)V");
	handle_cache.activity.onStart = _METHOD(handle_cache.activity.class, "performStart", "()V");
	handle_cache.activity.onWindowFocusChanged = _METHOD(handle_cache.activity.class, "onWindowFocusChanged", "(Z)V");
	handle_cache.activity.onResume = _METHOD(handle_cache.activity.class, "performResume", "()V");
	handle_cache.activity.onPostResume = _METHOD(handle_cache.activity.class, "onPostResume", "()V");
	handle_cache.activity.onDestroy = _METHOD(handle_cache.activity.class, "performDestroy", "()V");
	handle_cache.activity.onStop = _METHOD(handle_cache.activity.class, "performStop", "()V");
	handle_cache.activity.onPause = _METHOD(handle_cache.activity.class, "performPause", "()V");
	...
	handle_cache.attribute_set.class = _REF(atl_find_class_or_exit(env, "android/util/AttributeSet"));
	...
	handle_cache.array_list.class = _REF(atl_find_class_or_exit(env, "java/util/ArrayList"));
	...
	handle_cache.paint.class = _REF(atl_find_class_or_exit(env, "android/graphics/Paint"));
	handle_cache.paint.getColor = _METHOD(handle_cache.paint.class, "getColor", "()I");

	handle_cache.motion_event.class = _REF(atl_find_class_or_exit(env, "android/view/MotionEvent"));
	...
	handle_cache.sensor_event.class = _REF(atl_find_class_or_exit(env, "android/hardware/SensorEvent"));
	handle_cache.sensor_event.constructor = _METHOD(handle_cache.sensor_event.class, "<init>", "([FLandroid/hardware/Sensor;)V");

	handle_cache.audio_track_periodic_listener.class = _REF(atl_find_class_or_exit(env, "android/media/AudioTrack$OnPlaybackPositionUpdateListener"));
	...
	handle_cache.input_queue_callback.class = _REF(atl_find_class_or_exit(env, "android/view/InputQueue$Callback"));
	...
	handle_cache.surface_view.class = _REF(atl_find_class_or_exit(env, "android/view/SurfaceView"));
	...
	handle_cache.view.class = _REF(atl_find_class_or_exit(env, "android/view/View"));
	...
	handle_cache.view_group.class = _REF(atl_find_class_or_exit(env, "android/view/ViewGroup"));
	...
	handle_cache.asset_manager.class = _REF(atl_find_class_or_exit(env, "android/content/res/AssetManager"));
	...
	handle_cache.context.class = _REF(atl_find_class_or_exit(env, "android/content/Context"));
	...
	handle_cache.application.class = _REF(atl_find_class_or_exit(env, "android/app/Application"));
	handle_cache.application.get_app_icon_path = _METHOD(handle_cache.application.class, "get_app_icon_path", "()Ljava/lang/String;");

	handle_cache.looper.class = _REF(atl_find_class_or_exit(env, "android/os/Looper"));
	...
	handle_cache.key_event.class = _REF(atl_find_class_or_exit(env, "android/view/KeyEvent"));
	handle_cache.key_event.constructor = _METHOD(handle_cache.key_event.class, "<init>", "(JJIIII)V");

	handle_cache.intent.class = _REF(atl_find_class_or_exit(env, "android/content/Intent"));
	...
	handle_cache.instrumentation.class = _REF(atl_find_class_or_exit(env, "android/app/Instrumentation"));

	handle_cache.webview.class = _REF(atl_find_class_or_exit(env, "android/webkit/WebView"));
	...
	handle_cache.canvas.class = _REF(atl_find_class_or_exit(env, "android/graphics/Canvas"));
	...
	handle_cache.uri.class = _REF(atl_find_class_or_exit(env, "android/net/Uri"));
	handle_cache.uri.parse = _STATIC_METHOD(handle_cache.uri.class, "parse", "(Ljava/lang/String;)Landroid/net/Uri;");

	handle_cache.bundle.class = _REF(atl_find_class_or_exit(env, "android/os/Bundle"));
	...
	handle_cache.set.class = _REF(atl_find_class_or_exit(env, "java/util/Set"));
	handle_cache.set.toArray = _METHOD(handle_cache.set.class, "toArray", "()[Ljava/lang/Object;");

	handle_cache.parcel.class = _REF(atl_find_class_or_exit(env, "android/os/Parcel"));
	...
	handle_cache.builder_parcel.class = _REF(atl_find_class_or_exit(env, "android/atl/GVariantBuilderParcel"));
	handle_cache.builder_parcel.constructor = _METHOD(handle_cache.builder_parcel.class, "<init>", "(J)V");

	handle_cache.iter_parcel.class = _REF(atl_find_class_or_exit(env, "android/atl/GVariantIterParcel"));
	handle_cache.iter_parcel.constructor = _METHOD(handle_cache.iter_parcel.class, "<init>", "(J)V");

	handle_cache.view_tree_observer.class = _REF(atl_find_class_or_exit(env, "android/view/ViewTreeObserver"));
	...
	handle_cache.time_picker.class = _REF(atl_find_class_or_exit(env, "android/widget/TimePicker"));
	handle_cache.time_picker.onTimeChange = _METHOD(handle_cache.time_picker.class, "onTimeChange", "()V");

	handle_cache.date_picker.class = _REF(atl_find_class_or_exit(env, "android/widget/DatePicker"));
	handle_cache.date_picker.onDateChange = _METHOD(handle_cache.date_picker.class, "onDateChange", "()V");

	handle_cache.media_player.class = _REF(atl_find_class_or_exit(env, "android/media/MediaPlayer"));
	handle_cache.media_player.postEventFromNative = _METHOD(handle_cache.media_player.class, "postEventFromNative", "(III)V");

	handle_cache.bitmap.class = _REF(atl_find_class_or_exit(env, "android/graphics/Bitmap"));
	handle_cache.bitmap.fromNative = _STATIC_METHOD(handle_cache.bitmap.class, "fromNative", "(J)Landroid/graphics/Bitmap;");

	handle_cache.surface.class = _REF(atl_find_class_or_exit(env, "android/view/Surface"));

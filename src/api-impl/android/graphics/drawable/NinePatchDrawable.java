...
public class NinePatchDrawable extends Drawable {
	...
	/**
	 * ATL: create a drawable from a compiled .9.png on disk (the npTc chunk is
	 * parsed out of the file). Replaces the AOSP ImageDecoder ninepatch path.
	 *
	 * @hide
	 */
	public NinePatchDrawable(String path) {
		final Bitmap bitmap = BitmapFactory.decodeFile(path);
		final byte[] chunk = bitmap != null ? NinePatch.nativeReadChunkFromFile(path) : null;
		if (chunk == null) {
			throw new IllegalArgumentException("not a compiled nine-patch: " + path);
		}
		final NinePatch np = new NinePatch(bitmap, chunk, path);
		final Rect padding = new Rect();
		np.getPadding(padding);
		mNinePatchState = new NinePatchState(np, padding);
	...
	public void draw(Canvas canvas) {
		...
		final boolean needsDensityScaling = canvas.getDensity() == 0 && Bitmap.DENSITY_NONE != state.mNinePatch.getDensity();
	...
	private void updateStateFromTypedArray(@NonNull TypedArray a) throws XmlPullParserException {
		...
		if (srcResId != 0) {
			/* ATL: AOSP decodes through ImageDecoder here; we materialize the
			 * compiled .9.png and parse the chunk from the file */
			String file = null;
			try {
				final TypedValue value = new TypedValue();
				r.getValueForDensity(srcResId, 0, value, true);
				file = value.string != null ? value.string.toString() : null;
			} catch (Exception e) {
			...
			Drawable dr = null;
			if (file != null) {
				try (InputStream is = r.openRawResource(srcResId)) {
					dr = Drawable.createFromResourceStream(r, null, is, file, null);
				} catch (IOException e) {
					// Ignore and fail below.
				}
			}
			...
			final NinePatchState srcState = ((NinePatchDrawable)dr).mNinePatchState;
			state.mNinePatch = srcState.mNinePatch;
			state.mPadding = srcState.mPadding;
			state.mOpticalInsets = srcState.mOpticalInsets;
		}
	...
	public int getOpacity() {
		return mNinePatchState.mNinePatch == null || mNinePatchState.mNinePatch.hasAlpha() || (mPaint != null && mPaint.getAlpha() < 255) ? PixelFormat.TRANSLUCENT : PixelFormat.OPAQUE;
	}

	@Override
	public Region getTransparentRegion() {
		return mNinePatchState.mNinePatch != null
		           ? mNinePatchState.mNinePatch.getTransparentRegion(getBounds())
		           : null;
	}
	...
	private void computeBitmapSize() {
		...
		final int sourceDensity = ninePatch.getDensity() == Bitmap.DENSITY_NONE ? targetDensity : ninePatch.getDensity();
		...
		/* ATL: outline insets need Bitmap.getNinePatchInsets (aapt outline
		 * data); not available, so getOutline falls back to the bounds */
	}

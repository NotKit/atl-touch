...
EGLDisplay bionic_eglGetDisplay(NativeDisplayType native_display)
{
	...
	if (!native_display) {
		/* ... when there is one: GLFW only has an EGL display if it created
		 * its contexts through EGL, and on X11 it uses GLX. Handing the app
		 * EGL_NO_DISPLAY there would leave it with no way to get GL at all,
		 * and it does not need to share the window's display -- nothing
		 * passes buffers between them. */
		EGLDisplay display = atl_primary_egl_display();

		return display != EGL_NO_DISPLAY ? display : eglGetDisplay(EGL_DEFAULT_DISPLAY);
	}
